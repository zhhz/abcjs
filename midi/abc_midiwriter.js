...
(function() {
  ...
  // Midi facade: Objects which are instances of qtmidi or realtime midi respond to: setTempo, startTrack, endTrack, setInstrument, startNote, endNote, addRest, embed
  
  /**
   * internal to ABCJS.midi
   * Simultaneously creates a midi file, for downloading, and a realtimemidi, for playing
   * Implements the Midi facade
   */
  ...
  /** noplayer = only create the download link
   *
   */
  ...
  ABCJS.midi.MidiWriter.prototype.writeABC = function(abctune) {
    try {
      this.midi = (this.timbremidi) ? new MidiProxy(new ABCJS.midi.TimbreMidi(this, this.midijsparams), new Midi()) : new Midi();
      this.baraccidentals = [];
  ...
  ABCJS.midi.MidiWriter.prototype.writeNote = function(elem) {
	...
	if (note.startTie) {
	  ...
	  // only consider this slur a tie if has the same pitch and is the next note and is endslur
	  this.sameslurduration=0;
	  var nextnoteelem = this.getNextElemWithPitches();
	  if (nextnoteelem) { 
	    for (var j=0; j<nextnoteelem.pitches.length; j++) {
	      if (nextnoteelem.pitches[j].pitch === pitch && nextnoteelem.pitches[j].endSlur) { //TODO-GD cross bar accidental to non accidental or vice versa will f up.
	...
	if (note.endTie) {
	  ...
	  this.tieduration=0;
	} else if (note.endSlur && this.sameslurduration) {
	  this.midi.endNote(midipitches[i],mididuration+this.sameslurduration);
	  this.sameslurduration=0;
	} else {
